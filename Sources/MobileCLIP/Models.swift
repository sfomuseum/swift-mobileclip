...
nonisolated(unsafe) public let models: [ModelConfiguration] = [
...
nonisolated(unsafe) public let defaultModel = ModelConfiguration(name: "MobileCLIP-S2", factory: { S2Model() })
    ...
    */
    ...
    func encode(image: CVPixelBuffer) async -> Result<MLMultiArray, Error> {
        do {
            ...
            let enc = try mobileclip_s0_image()
    ...
    func encode(text: MLMultiArray) async  -> Result<MLMultiArray, Error> {
        do {
            ...
            let enc = try mobileclip_s0_text()
...
public struct S1Model: CLIPEncoder {
    ...
    func encode(image: CVPixelBuffer) async  -> Result<MLMultiArray, Error> {
        do {
            // let rsp = try await imageEncoder.get().prediction(image: image).final_emb_1
            let enc = try mobileclip_s1_image()
            let rsp = try enc.prediction(image: image).final_emb_1
            return .success(rsp)
        } catch {
            return .failure(error)
    ...
    func encode(text: MLMultiArray) async  -> Result<MLMultiArray, Error> {
        do {
            // let rsp = try await textEncoder.get().prediction(text: text).final_emb_1
            let enc = try mobileclip_s1_text()
            let rsp = try enc.prediction(text: text).final_emb_1
            return .success(rsp)
        } catch {
            return .failure(error)
