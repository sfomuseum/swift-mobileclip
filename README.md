...
gRPC server for deriving vector embeddings from an image or text.

```
$> embeddings-grpcd -h
USAGE: embeddings-grpcd <subcommand>
...
SUBCOMMANDS:
  ...
  See 'embeddings-grpcd help <subcommand>' for detailed help.
```
...
The length of texts used to generate text embeddings is capped at 77. This number _appears_ to be hardcoded in the MobileCLIP models themselves. I don't know why. I would like this number to be configurable but haven't figured out how yet. This is being tracked in [issue #1](https://github.com/sfomuseum/swift-mobileclip/issues/1).
...
This is not great from a performance perspective but after a number of attempts at finding ways to cache those implementations while making the compiler happy I have set the problem aside for the time being. Any pointers or suggestions would be greatly appreciated. This is being tracked in [issue #2](https://github.com/sfomuseum/swift-mobileclip/issues/2).

### No support for MobileCLIP2

As mentioned above there is not support for the MobileCLIP2 models yet. I would like to add them but I need to figure out how to convert in to CoreML models. Again, any pointers or suggestions would be greatly appreciated. This is being tracked in [issue #3](https://github.com/sfomuseum/swift-mobileclip/issues/3).
